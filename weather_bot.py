...
def get_live_poly_data():
    """משיכת נתונים עם ניסיון לדייק את המחיר מול פולימרקט"""
    try:
        ts = int(time.time())
        ...
        if response and len(response) > 0:
            ...
            for m in markets:
                ...
                try:
                    ...
                    # ניסיון למשוך את המחיר המדויק ביותר (Order Book Price)
                    prices = eval(m.get('outcomePrices', '["0.5", "0.5"]'))
                    price_raw = float(prices[0]) # מחיר ה-YES הנוכחי
...
def run_bot():
    ...
    <style>
        ...
        .title {{ font-size: 13px; color: #777; margin-bottom: 10px; font-weight: bold; text-transform: uppercase; border-bottom: 1px solid #222; padding-bottom: 5px; }}
        ...
        .footer {{ text-align: center; font-size: 11px; color: #444; margin-top: 30px; padding-bottom: 20px; }}
    </style></head>
    <body>
        ...
        <div class="card">
            ...
            <div class="desc-box">זהו הערך הממוצע שמתקבל משקלול 5 מודלים (ECMWF, UKMO, GFS, ICON, MeteoFrance). הוא מהווה את "נקודת האמת" של ה-AI שלנו.</div>
        </div>
        ...
        <div class="card">
            ...
            <table>
                <tr style="color:#555;">
                    ...
                    <th style="text-align:center;">שוק (מחיר|%)</th>
                    <th style="text-align:center;">שלנו (AI)</th>
                    <th style="text-align:left;">פער</th>
                </tr>
                {rows}
            </table>
            <div class="desc-box">
                <b>כיצד לקרוא:</b> פער חיובי (ירוק) אומר שה-AI מעריך שהסיכוי גבוה יותר ממה שהשוק מתמחר. <br>
                <b>האות (Signal):</b> נדלק ל-YES רק אם באופציה בעלת ההסתברות הגבוהה ביותר שלנו קיים פער חיובי מול השוק.
            </div>
        </div>
        ...
        <div class="card">
            ...
            <table>
                {" ".join([f"<tr><td>{n}</td><td style='color:{brand_green}; text-align:center;'>{t}°C</td><td style='text-align:left; color:#555;'>{int(weights[n]*100)}% משקל</td></tr>" for n,t in points.items()])}
            </table>
            <div class="desc-box">המשקלים נקבעים לפי רמת הדיוק ההיסטורית של כל מודל באזור לונדון.</div>
        </div>
