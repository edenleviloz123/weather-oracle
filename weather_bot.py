...
def get_live_poly_data():
    """משיכת נתונים עם מנגנון עקיפת Cache"""
    try:
        # הוספת timestamp לכתובת כדי להכריח את השרת לתת נתון טרי
        ts = int(time.time())
        ...
        for m in markets:
            ...
            try:
                ...
                # שימוש ב-lastTradePrice לקבלת המחיר הכי עדכני שהיה
                price_raw = float(m.get('lastTradePrice') or m.get('outcomePrices')[0])
...
def run_bot():
    ...
    poly_data = get_live_poly_data() or []
    ...
    # לוגיקה: Signal YES רק אם האופציה הכי סבירה של ה-AI היא עם Edge חיובי
    ...
    <body>
        ...
        <div class="card"><div class="title">🎯 תחזית Oracle</div><div class="main-val">{avg_val:.2f}°C</div></div>
        ...
        <div style="text-align: center; font-size: 11px; color: #444;">🕒 IL: {now_il} | UK: {now_uk}</div>
    </body></html>"""
