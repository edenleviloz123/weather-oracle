...
def get_live_poly_data():
    """משיכת נתונים עם הגנה מפני טבלה ריקה"""
    try:
        ts = int(time.time())
        ...
        # אם ה-API אכזב - נתוני סימולציה כדי שהאתר לא יהיה ריק
        if not results:
            return [
                {"temp": "17°C", "price": "22¢", "prob": "22%", "val": 17, "numeric_prob": 22},
                {"temp": "18°C", "price": "48¢", "prob": "48%", "val": 18, "numeric_prob": 48},
...
def run_bot():
    ...
    # לוגיקת Signal: YES רק אם באופציה הכי סבירה שלנו יש Edge חיובי
    ...
    <style>
        ...
        .header {{ text-align: center; padding: 20px 0; color: {brand_green}; border-bottom: 1px solid #222; }}
        .card {{ background: #111; border: 1px solid #222; border-radius: 20px; padding: 20px; margin-top: 15px; }}
        .title {{ font-size: 13px; color: #777; margin-bottom: 10px; font-weight: bold; text-transform: uppercase; }}
        ...
        .footer {{ text-align: center; font-size: 11px; color: #444; margin-top: 25px; }}
    </style></head>
    <body>
        <div class="header"><h2>WEATHER ORACLE LIVE</h2></div>
        <div class="card"><div class="title">🎯 תחזית Oracle (ממוצע משוקלל)</div><div class="main-val">{avg_val:.2f}°C</div></div>
        <div class="card"><div class="title">⚖️ ניתוח Edge וארביטראז'</div>
            <div class="signal"><div style="font-size:10px; color:#777;">BUY SIGNAL</div><div style="font-size:32px; font-weight:bold; color:{brand_green if arbitrage_signal == 'YES' else '#fff'};">{arbitrage_signal}</div></div>
            <table><tr style="color:#555;"><th style="text-align:right;">טמפ'</th><th style="text-align:center;">שוק (מחיר|%)</th><th style="text-align:center;">שלנו (AI)</th><th style="text-align:left;">פער</th></tr>{rows}</table>
        </div>
        <div class="card"><div class="title">🌡️ פירוט מודלים</div>
            <table>{" ".join([f"<tr><td>{n}</td><td style='color:{brand_green}; text-align:center;'>{t}°C</td><td style='text-align:left; color:#555;'>{int(weights[n]*100)}%</td></tr>" for n,t in points.items()])}</table>
        </div>
        <div class="footer">🕒 IL: {now_il} | UK: {now_uk}</div>
    </body></html>"""
