...
def get_market_data():
    """משיכת נתונים מפולימרקט באמצעות ה-Secret שהגדרנו"""
...
def run_bot():
    ...
    <style>
        ...
        .title {{ font-size:10px; color:#444; font-weight:bold; margin-bottom:10px; }}
        table {{ width:100%; border-collapse:collapse; }}
        th {{ font-size:9px; color:#333; padding-bottom:8px; }}
    </style></head>
    <body>
        <div style="text-align:center; color:{BRAND_GREEN}; letter-spacing:3px; padding:10px; font-weight:bold;">ORACLE MONSTER v4.1</div>
        ...
        <div class="card">
            ...
            <div style="display:flex; justify-content:space-around;">
                ...
                <div style="text-align:center;"><small style="color:#444;">LHR LIVE</small><br><b style="font-size:20px;">{lhr_live}° {momentum}</b></div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            {f"<table><tr><th style='text-align:right;'>מעלות</th><th>פולי</th><th>AI %</th><th style='text-align:left;'>EDGE</th></tr>{rows}</table>" if processed else f"<div style='color:{ERROR_RED}; text-align:center;'>⚠️ {err if err else 'ממתין לנתוני שוק...'}</div>"}
        </div>
